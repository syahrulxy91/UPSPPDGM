...
exports.lookupBoundInstitutionForGoogleUser = exports.getPublicInstitutionsList = exports.deleteInstitutionCompletely = exports.updateInstitutionActiveStatus = exports.resetInstitutionBinding = exports.bindOrValidateInstitutionAccess = exports.resetInstitutionGoogleEmail = exports.bindInstitutionGoogleEmail = void 0;
const https_1 = require("firebase-functions/v2/https");
const admin = __importStar(require("firebase-admin"));
/**
 * Helper to log security actions into the audit log ledger
 */
async function logSecurityEvent(db, institusiId, namaInstitusi, actionType, description, performedByEmail, metadata) {
    try {
 ...
 */
exports.bindInstitutionGoogleEmail = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Sila log masuk dengan akaun Google terlebih dahulu.");
    }
    const data = request.data || {};
    const { institusiId } = data;
    if (!institusiId) {
        throw new https_1.HttpsError("invalid-argument", "Sila sertakan 'institusiId'.");
    }
    const email = request.auth.token.email;
    const uid = request.auth.uid;
    if (!email) {
        throw new https_1.HttpsError("failed-precondition", "Akaun Google anda tidak mempunyai alamat emel yang sah.");
    }
    const db = admin.firestore();
    const institusiRef = db.collection("institusi").doc(institusiId);
    const docSnap = await institusiRef.get();
    if (!docSnap.exists) {
        throw new https_1.HttpsError("not-found", "Institusi tidak ditemui.");
    }
    ...
    if (!pAcc) {
        throw new https_1.HttpsError("permission-denied", "Akses portal bagi institusi ini belum diaktifkan oleh pegawai PPDGM.");
    }
    if (pAcc.enabled !== true || pAcc.credentialStatus !== "aktif") {
        throw new https_1.HttpsError("permission-denied", "Akses portal bagi institusi ini belum diaktifkan atau telah disekat. Sila hubungi pegawai PPDGM.");
    }
    // Check if the email is authorized
    const authorizedEmails = pAcc.googleAuthorizedEmails || [];
    const normalizedEmail = email.toLowerCase().trim();
    const isAuthorized = authorizedEmails.some(ae => ae.toLowerCase().trim() === normalizedEmail);
    if (!isAuthorized) {
        throw new https_1.HttpsError("permission-denied", `Emel Google anda (${email}) tidak tersenarai antara emel yang dibenarkan untuk mengakses portal institusi ${namaInstitusi}. Sila hubungi pegawai PPDGM.`);
    }
    ...
    if (boundEmail) {
        ...
        if (normalizedBound !== normalizedEmail) {
            throw new https_1.HttpsError("permission-denied", `Ralat: Institusi ini telah dipautkan secara kekal kepada emel Google yang lain (${boundEmail}). Pengguna lain tidak dibenarkan masuk.`);
        }
        ...
        // Auditing
        await logSecurityEvent(db, institusiId, namaInstitusi, "paut_emel_google", `[Pautan Pertama] Berjaya memautkan emel Google ${email} kepada IPS: ${namaInstitusi}.`, email, { email, uid, boundBy: "self-first-login", bindingLocked: true });
 ...
 */
exports.resetInstitutionGoogleEmail = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Sila log masuk dengan akaun Google terlebih dahulu.");
    }
    const email = request.auth.token.email;
    ...
    const pAcc = institusiData.portalAccess;
    if (!pAcc) {
        throw new https_1.HttpsError("failed-precondition", "Akses portal bagi institusi ini belum diaktifkan.");
    }
    ...
    await institusiRef.set({ portalAccess: updatedPortalAccess }, { merge: true });
    // Auditing
    await logSecurityEvent(db, institusiId, namaInstitusi, "reset_paut_emel_google", `[Set Semula Pautan] Pegawai ${performedBy || email || "Sistem"} menetapkan semula pautan emel Google bagi IPS: ${namaInstitusi}.`, performedBy || email || "Sistem", { resetBy: performedBy || email, resetAt: new Date().toISOString() });
    return { success: true };
});
/**
 ...
 */
exports.bindOrValidateInstitutionAccess = (0, https_1.onCall)(async (request) => {
    const data = request.data || {};
    const { institusiId, email, uid, displayName } = data;
    if (!institusiId || !email || !uid) {
        throw new https_1.HttpsError("invalid-argument", "Sila sertakan 'institusiId', 'email', dan 'uid'.");
    }
    const normalizedEmail = email.toLowerCase().trim();
    const db = admin.firestore();
    const institusiRef = db.collection("institusi").doc(institusiId);
    try {
        const result = await db.runTransaction(async (transaction) => {
            ...
            if (isExplicitlyBlocked) {
                throw new https_1.HttpsError("permission-denied", `Akses portal bagi institusi '${namaInstitusi}' telah disekat atau belum diaktifkan. Sila hubungi Pegawai Swasta PPD Gua Musang.`);
            }
            ...
            if (!boundGoogleEmail) {
                ...
                if (normalizedBound === normalizedEmail) {
                    ...
                    throw new https_1.HttpsError("permission-denied", `Institusi ini telah dipautkan kepada akaun Google yang lain. Sila gunakan akaun yang telah dipautkan atau hubungi pegawai PPDGM untuk mendapat pelepasan semula.`);
                }
            }
        });
        if (result.bindingCreated) {
            await logSecurityEvent(db, institusiId, result.namaInstitusi, "paut_emel_google", `[Pautan Pertama] Berjaya memautkan emel Google ${normalizedEmail} kepada IPS secara automatik semasa log masuk kali pertama.`, normalizedEmail, { email: normalizedEmail, uid, boundBy: "self-first-login", bindingLocked: true });
        }
        return {
            success: true,
        ...
        if (err instanceof https_1.HttpsError || err.code === "permission-denied" || err.code === "invalid-argument" || err.code === "not-found") {
            throw new https_1.HttpsError(err.code || "permission-denied", err.message);
        }
        throw new https_1.HttpsError("failed-precondition", `Error Debug: ${err.message || err}`);
    }
});
/**
 ...
 */
exports.resetInstitutionBinding = (0, https_1.onCall)(async (request) => {
    const data = request.data || {};
    const { institusiId, performedBy } = data;
    if (!institusiId) {
        throw new https_1.HttpsError("invalid-argument", "Sila sertakan 'institusiId'.");
    ...
    delete updatedPortalAccess.authUid;
    await institusiRef.set({ portalAccess: updatedPortalAccess }, { merge: true });
    await logSecurityEvent(db, institusiId, namaInstitusi, "reset_paut_emel_google", `[Set Semula Pautan] Pegawai ${performedBy || "Sistem"} menetapkan semula pautan emel Google bagi IPS: ${namaInstitusi}.`, performedBy || "Sistem", { resetBy: performedBy || "Sistem", resetAt: new Date().toISOString() });
    return { success: true };
});
/**
 ...
 */
exports.updateInstitutionActiveStatus = (0, https_1.onCall)(async (request) => {
    const data = request.data || {};
    const { institusiId, enabled, performedBy } = data;
    if (!institusiId || enabled === undefined) {
        throw new https_1.HttpsError("invalid-argument", "Sila nyatakan 'institusiId' dan parameter 'enabled'.");
    }
    const db = admin.firestore();
    const institusiRef = db.collection("institusi").doc(institusiId);
    const docSnap = await institusiRef.get();
    ...
    await logSecurityEvent(db, institusiId, namaInstitusi, enabled ? "aktifkan_akses" : "menyekat_akses", `[Status Capaian] Menukar status capaian portal kepada [${statusVal.toUpperCase()}] bagi IPS: ${namaInstitusi}.`, performedBy || "pegawai@ppdgm.gov.my", { enabled, statusOperasi: statusVal });
    return { success: true };
});
/**
 ...
 */
exports.deleteInstitutionCompletely = (0, https_1.onCall)(async (request) => {
    const data = request.data || {};
    const { institusiId, performedBy } = data;
    if (!institusiId) {
        throw new https_1.HttpsError("invalid-argument", "Sila sertakan 'institusiId'.");
 ...
 */
exports.getPublicInstitutionsList = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Sila log masuk dengan akaun Google terlebih dahulu.");
    }
    const db = admin.firestore();
    try {
        ...
        throw new https_1.HttpsError("internal", "Gagal memuat senarai institusi awam.");
    }
});
/**
 ...
 */
exports.lookupBoundInstitutionForGoogleUser = (0, https_1.onCall)(async (request) => {
    const data = request.data || {};
    const { email, uid } = data;
    if (!email) {
        throw new https_1.HttpsError("invalid-argument", "Sila sertakan parameter 'email'.");
    }
    const normalizedEmail = email.toLowerCase().trim();
    const db = admin.firestore();
    try {
        ...
        if (uid) {
            queries.push(db.collection("institusi").where("portalAccess.boundGoogleUid", "==", uid).get(), db.collection("institusi").where("portalAccess.authUid", "==", uid).get());
        }
        ...
        throw new https_1.HttpsError("internal", `Ralat semasa menyemak pautan: ${err.message || err}`);
    }
});
