...
import { ComplianceActionPanel } from "../../../shared/components/ui/ComplianceActionPanel";
...
export function DashboardUtamaPage() {
  ...
  if (loading) {
    return (
      <div className="space-y-8 animate-pulse p-4 md:p-5 lg:p-6 max-w-[1600px] mx-auto" id="dashboard-loading">
  ...
  return (
    <div className="space-y-8 max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 py-4 pb-20" id="dashboard-success-view">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5" id="kpi-grid">
        ...
        <KpiCard
          label="Institusi Berdaftar"
          value={overview.jumlahInstitusiAktif}
          hint="Sedia beroperasi di daerah Gua Musang"
          tone="default"
        />
        ...
        <KpiCard
          label="Kes Aktif"
          value={overview.kesAktif}
          hint="Aduan & semakan tindakan baru"
          tone="warning"
        />
        ...
        <KpiCard
          ...
          value={overview.tindakanSegera}
          hint="Dokumen gagal mematuhi dikesan"
          tone="danger"
        />
        ...
        <KpiCard
          label="Hampir Luput"
          value={overview.dokumenHampirLuput}
          hint="Lesen/permit tamat tempoh n/q"
          tone="warning"
        />
        ...
        <KpiCard
          ...
          value={overview.selesaiBulanIni}
          hint="Melebihi sasaran KPI bulanan"
          tone="success"
        />
      ...
      <div className="border border-slate-250/60 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300">
        <ComplianceActionPanel 
          ...
          onOpenTindakanPortal={() => navigate("/tindakan")}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="dashboard-bento-ops">
        ...
        {/* LOG TINDAKAN & AKTIVITI SEMASA (1/3) */}
        <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 space-y-6 flex flex-col justify-between shadow-3xs" id="stream-activity">
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-150/60 pb-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-[#01696f]/10 text-[#01696f] rounded-xl">
                  <Activity className="w-4 h-4 animate-pulse" />
                </div>
                <div className="text-left">
                  <h3 className="font-extrabold text-slate-900 text-xs uppercase tracking-wider">Log Tindakan & Aktiviti</h3>
                  <p className="text-[9px] text-[#01696f] font-black uppercase tracking-widest mt-0.5">
                    KRONOLOGI LIVE
                  </p>
                </div>
              </div>
            ...
            <div className="max-h-80 overflow-y-auto pr-2 text-left custom-scrollbar relative pl-3 py-1">
              {overview.aktivitiTerkini.length === 0 ? (
                ...
                <div className="relative border-l border-slate-100 space-y-4">
                  {[...overview.aktivitiTerkini]
                    .sort((a, b) => b.masa.localeCompare(a.masa))
                    .slice(0, 10)
                    .map((act) => (
                      <div key={act.id} className="relative pl-4 group transition-all duration-200">
                        
                        {/* Bullet indicator on the timeline axis */}
                        <span className="absolute -left-[5px] top-1.5 w-2.5 h-2.5 bg-white border-2 border-[#01696f] rounded-full flex items-center justify-center shadow-3xs group-hover:bg-[#01696f] transition-all">
                          <span className="w-1 h-1 bg-[#01696f] group-hover:bg-white rounded-full transition-all" />
                        </span>

                        <div className="space-y-2 bg-slate-50/50 hover:bg-slate-50 border border-slate-150 group-hover:border-slate-250 p-3 rounded-xl transition-all shadow-3xs hover:shadow-2xs">
                          {/* Title of active change log */}
                          <h4 className="text-[12px] font-black text-slate-900 leading-snug tracking-tight group-hover:text-[#01696f] transition-colors">
                            {act.tajuk}
                          </h4>
                          
                          <div className="flex flex-col gap-1.5 pt-1.5 border-t border-slate-150/40">
                            {/* Verified system agent or officer */}
                            <div className="flex items-center gap-1 text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                              <span className="text-[#01696f] font-black text-[8.5px]">PENGESAH:</span>
                              <span className="truncate max-w-[120px] text-slate-700 font-mono font-bold">
                                {act.pegawai || "Sistem"}
                              </span>
                            </div>

                            {/* Cap masa / tarikh */}
                            <div className="flex items-center gap-1 text-slate-450 text-[9.5px] font-mono font-black">
